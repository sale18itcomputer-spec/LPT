...
- Top Performing Product Lines: ${historicalAnalysis.topProductLines.map(p => `${p.name} ($${p.revenue.toFixed(2)})`).join(', ') || 'None'}
            ...
            <div className="pt-4">
                <AnimatePresence mode="wait">
                    {isLoading ? (
                        ...
                        >
                            <p className="font-semibold">Analyzing historical data...</p>
                            <p className="text-sm">Gemini is crafting tailored campaigns.</p>
                        </motion.div>
                    ) : error ? (
                        <motion.div 
                            key="error"
