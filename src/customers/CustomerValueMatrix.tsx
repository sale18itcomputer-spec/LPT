...
}> = ({ title, description, icon: Icon, items, bgGradientClass, borderColorClass, onSelectCustomer }) => {
    const totalRevenue = useMemo(() => items.reduce((sum, item) => sum + item.totalRevenue, 0), [items]);
    
    return (
        <div className={`rounded-xl p-3 sm:p-4 flex flex-col h-80 sm:h-96 ${bgGradientClass} border ${borderColorClass} shadow-inner`}>
            ...
            <ul className="flex-grow overflow-y-auto pr-2 -mr-2 text-sm custom-scrollbar divide-y divide-black/5 dark:divide-white/5">
                 {items.length > 0 ? (
                    items.map(customer => (
                        <li key={customer.id} className="py-1.5">
                            <button
...
const CustomerValueMatrix: React.FC<{ customers: Customer[]; onSelectCustomer: (customer: Customer) => void; }> = ({ customers, onSelectCustomer }) => {
